...
query <- paste0( "select t1.subscription_id, t1.plan, substring_index(substring_index(t1.plan, '-', -3),  '-', 1) as kits, substring_index(substring_index(t1.plan, '-', -2),  '-', 1) as serving_size,
 t1.scm_week_choice_for, t1.recipe
from (
SELECT
...
choiceDF <- bobDF_clean %>% group_by(modifiedOn, `Pull Week`, scm_week_choice_for, kits, serving_size) %>% 
  do({
    X <- data.frame(., stringsAsFactors = F)
...
names(choiceDF) <- c("Pull Date", "Pull Week", "HF Week", "Kits/Box", "Serving Size",
...
write_csv(choiceDF, path = "bobChoice.csv")
...
gs_upload("bobChoice.csv",sheet_title = "ep_forecasting_scripts_output_bobChoice", overwrite = T, verbose = T)
...
#names(defaultDF) <- c("HF Week", "Kits/Box", "Serving Size", "Count", "Share", "Week Num")
#defaultDF <- defaultDF %>% group_by(`HF Week`, `Kits/Box`, `Serving Size`, `Week Num`) %>% summarise(Count = sum(Count))


#options(scipen = 999)
#defaultDFPreferenceShare <- defaultDF %>%  group_by(`HF Week`, `Kits/Box`, `Serving Size`) %>% do({
...
#futurePreferenceShare <- defaultDFPreferenceShare %>% group_by(`Kits/Box`, `Serving Size`) %>% do({
# X <- data.frame(., stringsAsFactors = F)
...
#names(futurePreferenceShare) 
#futurePreferenceShareMatrix <- expand.grid(unlist(weeksAheadList),  unique(futurePreferenceShare$`Kits/Box`), unique(futurePreferenceShare$`Serving Size`))
#names(futurePreferenceShareMatrix)  <- c("HF Week", "Kits/Box", "Serving Size")
